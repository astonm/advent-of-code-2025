...
def read_file(input, delim="\n"):
    lines = [l.strip() for l in input.read().strip().split(delim)]
    end = first((i for i in reversed(range(len(lines))) if lines[i]), -1)
    return lines[: end + 1]
...
class Grid:
    ...
    def neighbors(self, p, udlr=True, diags=False):
        deltas = []
        if udlr:
            deltas = deltas + Grid.UDLR
        if diags:
